...
const raw_steps = [
    { name: 'Register Client', description: 'Create a client/application'},
...
export default {
    ...
    setup() {
      ...
      const current_step = computed(() => usePage().props.value.currentStep)

      const steps = computed(() => _.map((raw_steps), function (step,index) {

        return {
          ...step,
          status: current_step.value >= index+2
            ? 'complete'
            : current_step.value === index+1 ? 'current' : 'upcoming'
        }
      }))
