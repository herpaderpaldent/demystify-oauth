      ...
      <div class="relative bg-white shadow-xl">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3">
          ...
          <div class="py-10 px-6 sm:px-10 lg:col-span-2 xl:p-12">
            <h3 class="text-lg font-medium text-gray-900">Register an application</h3>
            ...
            <form @submit.prevent="form.post(route('create.client'))" method="POST" class="mt-6 grid grid-cols-1 gap-y-6">
              <div class="sm:col-span-2">
                <label for="application_name" class="block text-sm font-medium text-gray-900">Application Name</label>
                <div class="mt-1">
                  <input v-model="form.application_name" type="text" name="application_name" id="application_name" class="py-3 px-4 block w-full shadow-sm text-gray-900 focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md" />
                </div>
              </div>
              <div class="sm:col-span-2">
                <label for="callback_url" class="block text-sm font-medium text-gray-900">Authorization callback URL</label>
                <div class="mt-1">
                  <input v-model="form.callback_url" type="text" name="callback_url" id="callback_url" class="py-3 px-4 block w-full shadow-sm text-gray-900 focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md" />
                </div>
                <p class="mt-2 text-sm text-gray-500" id="callback_url-description">The callback URL is used to determine where the auth server redirects the user after they authorize your access request, and it can include query parameters. The redirect will include the same query parameters, as well as the access token, which your application must be able to parse.</p>
              </div>
              <div class="sm:col-span-2">
                <label for="email" class="block text-sm font-medium text-gray-900">Email</label>
                <div class="mt-1">
                  <input v-model="form.email" type="text" name="email" id="email" class="py-3 px-4 block w-full shadow-sm text-gray-900 focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md" />
                </div>
              </div>
              <div class="sm:col-span-2">
                ...
                <div class="mt-1">
                  <textarea v-model="form.description" id="message" name="message" rows="4" class="py-3 px-4 block w-full shadow-sm text-gray-900 focus:ring-indigo-500 focus:border-indigo-500 border border-gray-300 rounded-md" aria-describedby="message-max" />
                </div>
              </div>
              <div class="sm:col-span-2 sm:flex sm:justify-end">
                <button type="submit" :disabled="form.processing" class="mt-2 w-full inline-flex items-center justify-center px-6 py-3 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:w-auto">
                  Submit
                </button>
              </div>
            </form>
...
<script>
import {MailIcon, PhoneIcon} from "@heroicons/vue/outline/esm";
